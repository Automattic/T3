 ...
 */
export const ThemeGardenListItem = ( {
	theme: { activate_url: activateUrl, id, thumbnail, title },
	handleDetailsClick,
	isActive,
} ) => {
	...
	const renderActivationButton = () => {
		return isActive ? (
			...
			<a className="rainbow-button" href={ activateUrl }>
				Activate
			</a>
		);
	};
	...
	return (
		<article className={ classNames( 'tumblr-theme', { 'is-active': isActive } ) }>
			...
			<div className="tumblr-theme-content">
				{ renderPreviewButton() }
				<div className="tumblr-theme-footer">{ renderActivationButton() }</div>
			</div>
		</article>
	);
};
